...
function get_group($gid){
    ...
    for($i=0;$i<count($groups);$i++){
        $r[$groups[$i]['id']]=$groups[$i];
    }
    return $gid?$r[$gid]:$r;
}
function get_gender($gender){
    $genders=[1=>'男',0=>'女'];
    return $gender?$genders[$gender]:$genders;
}
function get_event_type($type){
    $types=[1=>'任务分',2=>'义务分',3=>'减分项'];
    return $type?$types[$type]:$types;
}
function get_event_group($gid){
    $groups=M("Group")->select();
    $r=[];
    $r[0]=['name'=>'全部小组'];
    for($i=0;$i<count($groups);$i++){
...
function get_task_status($status){
    $statuses=[0=>"进行中",1=>"已解决",2=>"撤销"];
    return $status?$statuses[$status]:$statuses;
}
function get_user_event_type($type){
    $types=[0=>"积分事件",1=>"积分任务"];
    return $types[$type];
}
function get_news_type(){
    return [1=>'支部公告',2=>'支部新闻'];
}
function time_format_str($time = NULL,$format='Y-m-d H:i'){
    $time = ($time === NULL || $time==='0000-00-00 00:00:00' || $time===0) ? '' : $time;
    if(!$time){
        return $time;
    }
...
function checkRealname($realname){
    if(!preg_match('/^[\x{4e00}-\x{9fa5}]{2,6}+$/u',$realname)){//匹配2到四个汉字  
        return false;
    }  
